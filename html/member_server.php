...
if(isset($_POST['ID']) && isset($_POST['password']) && isset($_POST['password_CK']) && isset($_POST['nickname']) && isset($_POST['user_name']) && isset($_POST['user_email']) && isset($_POST['user_phone'])){
    ...
    if(empty($ID)){//ID 대한 에러 체크    
        ...
        if(mysqli_num_rows($order_ID)>0){ //만약 아이디만 이미 존재한다면~
            ...
            $sql_save = "insert into member(ID, password, nickname, user_name, user_email, user_phone) values('$ID','$password','$nickname', '$user_name', '$user_email', '$user_phone')";
